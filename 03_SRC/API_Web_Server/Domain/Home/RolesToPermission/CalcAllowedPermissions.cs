...
namespace API_Web_Server.Domain.Home.RolesToPermission
{
    public class CalcAllowedPermissions : IDisposable
    {
        ...
        /// </summary>
        private readonly DbContextOptions<Api_ServerDbContext> _apisvDbContextOptions;

        private Api_ServerDbContext _context;

        public CalcAllowedPermissions(Api_ServerDbContext context)
        {
            _context = context;
        }

        public CalcAllowedPermissions(DbContextOptions<Api_ServerDbContext> apisvDbContextOptions)
        {
            _apisvDbContextOptions = apisvDbContextOptions;
        }
        ...
        private Api_ServerDbContext GetContext()
        {
            return _context ?? new Api_ServerDbContext(_apisvDbContextOptions);
        }
        ...
        {
            if (_apisvDbContextOptions != null && _context != null)
            {
                if (disposing)
                {
                    _context.Dispose();
